...
@NullMarked
class AzureOpenAIChatCompletionsServiceTest extends LLMServiceFacadeBasedUnitTest implements MixChatRequestMock {
    private final AzureOpenAIChatCompletionsService service;

    AzureOpenAIChatCompletionsServiceTest() throws NotConfiguredException {
        AzureOpenAIProviderConfigElement openai = ProviderConfigElement.load().azure().openai();

        service = new AzureOpenAIChatCompletionsService(openai, getUnitTestLogger());
    }
