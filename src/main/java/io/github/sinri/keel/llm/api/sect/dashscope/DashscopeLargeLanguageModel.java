    ...
    public LLMService getService() {
        if (modelCode.startsWith("qwen3-max")
            ...
            try {
                ...
                Logger logger = LLMServiceFacade.getInstance().getLogger();
                return new DashscopeTextGenerationService(apiKey, logger);
            ...
            try {
                String apiKey = getApiKey();
                Logger logger = LLMServiceFacade.getInstance().getLogger();
                return new DashscopeMultimodalGenerationService(apiKey, logger);
            } catch (NotConfiguredException e) {
                throw new RuntimeException(e);
            }
        } else {
