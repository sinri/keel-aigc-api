...
import io.github.sinri.keel.aigc.api.internal.llm.sect.dialect.doubao.DoubaoResponseChunkImpl;
import io.vertx.core.json.JsonObject;
import org.jspecify.annotations.Nullable;

import java.util.List;

public interface DoubaoResponseChunk extends UnmodifiableJsonifiableEntity {
    static DoubaoResponseChunk wrap(JsonObject jsonObject) {
        return new DoubaoResponseChunkImpl(jsonObject);
    }
     ...
     * @return 本次请求的模型输出内容。
     */
    default List<DoubaoResponseChunkChoice> getChoices() {
        List<JsonObject> choices = readJsonObjectArray("choices");
        if (choices == null) return List.of();
        return choices.stream().map(DoubaoResponseChunkChoice::wrap).toList();
    }
